CREATE TABLE [ods].[TM_Tkt_History]
(
...
GO
ALTER TABLE [ods].[TM_Tkt_History] ADD CONSTRAINT [PK__TM_Tkt_History] PRIMARY KEY NONCLUSTERED  ([id])
GO
CREATE NONCLUSTERED INDEX [IDX_BusinessKey] ON [ods].[TM_Tkt_History] ([acct_id], [plan_event_id], [ticket_status], [section_id], [row_id], [seat_num]) INCLUDE ([num_seats], [ssbPriceCode])
GO
CREATE NONCLUSTERED INDEX [IDX_ticket_status] ON [ods].[TM_Tkt_History] ([ticket_status])
GO
CREATE NONCLUSTERED INDEX [IDX_ticket_status_add_datetime] ON [ods].[TM_Tkt_History] ([ticket_status], [add_datetime]) INCLUDE ([block_purchase_price], [event_id], [event_name], [num_seats], [plan_event_id], [price_code], [Price_code_desc], [row_id], [seat_num], [section_id])
GO
CREATE CLUSTERED INDEX [Idx_upd_datetime_event_id] ON [ods].[TM_Tkt_History] ([upd_datetime], [acct_id], [event_id])
GO
CREATE NONCLUSTERED INDEX [IDX_UpdateDate] ON [ods].[TM_Tkt_History] ([UpdateDate])
GO
