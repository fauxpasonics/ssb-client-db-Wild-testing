...
CREATE VIEW [etl].[vw_Load_DimCustomer_NCR_Retail]
AS
...
SELECT  DB_NAME() AS [SourceDB]
      , 'NCR_Retail' AS [SourceSystem]
      , NULL AS [SourceSystemPriority]


/*Standard Attributes*/
      , CAST(a2.CUST_NO AS NVARCHAR(100)) AS [SSID]
      ...
      , CONVERT(VARCHAR(30),NULL, 102) AS [Birthday]
      , NULL AS [Gender]
      , 0 [MergedRecordFlag]
      , NULL [MergedIntoSSID]
...
/*Name*/
      , NULL AS [Prefix]
      , a2.FST_NAM AS [FirstName]
      , NULL AS [MiddleName]
      , a2.LST_NAM AS [LastName]
      , NULL AS [Suffix]
      , NULL AS [FullName]
--, c.name_title as [Title]

/*AddressPrimary*/
      , CASE WHEN a2.[ADRS_1] = '' THEN NULL
			ELSE a2.[ADRS_1] + '' + a2.[ADRS_2] END AS [AddressPrimaryStreet]
	  , NULL AS [AddressPrimarySuite]
      , CASE WHEN a2.CITY = '' THEN NULL 
			ELSE a2.CITY END AS [AddressPrimaryCity]
      , CASE WHEN a2.[STATE] = '' THEN NULL 
			ELSE a2.[STATE] END AS [AddressPrimaryState]
      , CASE WHEN a2.[ZIP_COD] = '' THEN NULL 
			ELSE a2.ZIP_COD END AS [AddressPrimaryZip]
      , NULL AS [AddressPrimaryCounty]
      , CASE WHEN a2.[CNTRY] = '' THEN NULL 
			ELSE a2.CNTRY END AS [AddressPrimaryCountry]
...
/*Phone*/
      , CASE WHEN a2.PHONE_1 = '' THEN NULL	
			ELSE a2.PHONE_1 END AS [PhonePrimary]
      , CASE WHEN a2.PHONE_2 = '' THEN NULL	
			ELSE a2.PHONE_2 END AS [PhoneHome]
      , NULL AS [PhoneCell]
...
/*Email*/
      , CASE WHEN a2.EMAIL_ADRS_1 = '' THEN NULL	
			ELSE a2.EMAIL_ADRS_1 END AS [EmailPrimary]
      , CASE WHEN a2.EMAIL_ADRS_1 = '' THEN NULL	
			ELSE a2.EMAIL_ADRS_1 END AS [EmailOne]
      , CASE WHEN a2.EMAIL_ADRS_2 = '' THEN NULL	
			ELSE a2.EMAIL_ADRS_2 END AS [EmailTwo]
...
/*Source Created and Updated*/
      , NULL AS [SSCreatedBy]
      , NULL AS [SSUpdatedBy]
      , GETDATE() [SSCreatedDate]
      , NULL [SSUpdatedDate]
      , a2.ETL_CreatedDate AS CreatedDate
      , a2.ETL_UpdatedDate AS UpdatedDate
      , ALT_CUST_NO AS [AccountId]
      , 0 IsBusiness
	  , 0 AS IsDeleted
	  , NULL AS [customer_matchkey]

--		select *
	FROM [ods].[NCR_Customers] AS a2 (NOLOCK)
	WHERE 1 = 1
