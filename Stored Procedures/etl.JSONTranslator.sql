...
*	5	12/08/2017		Tommy Dickinson			Implemented alternative (optimized) loading process for flat hierarchy data
***************************************************************************************************************************/
...
IF EXISTS (SELECT TOP 1 1 FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_SCHEMA in ('stg','apietl') AND TABLE_TYPE = 'BASE TABLE' AND Table_Name NOT IN (SELECT DISTINCT Name FROM [etl].[JSON_Meta_Table]))
BEGIN

SET @SQL = '
INSERT INTO [etl].[JSON_Meta_Table] ([Schema],Name)
SELECT TABLE_SCHEMA,Table_Name FROM INFORMATION_SCHEMA.Tables WHERE TABLE_SCHEMA IN (''stg'',''apietl'') AND TABLE_TYPE = ''BASE TABLE'' AND Table_Name NOT IN (SELECT DISTINCT Name FROM [etl].[JSON_Meta_Table])
...
SELECT
	...
	,tcc.Name_Raw
INTO #Metadata
FROM [etl].[JSON_Meta_Table_Configuration] tcon WITH (NOLOCK)
	JOIN [etl].[JSON_Meta_TableColumn_Configuration] tccon WITH (NOLOCK) ON tcon.JSON_Meta_Table_Configuration_ID = tccon.JSON_Meta_Table_Configuration_ID
	LEFT JOIN [etl].[JSON_Meta_TableColumn] tcc WITH (NOLOCK) ON tcon.JSON_Meta_Table_ID = tcc.JSON_Meta_Table_ID AND tccon.JSON_Meta_TableColumn_ID = tcc.JSON_Meta_TableColumn_ID
...
IF @Step = 'Create SRC Table (Flat)'
BEGIN

IF OBJECT_ID('tempdb.dbo.#Metadata','U') IS NOT NULL DROP TABLE #MetadataFlat
SELECT
	tcon.TargetSchema
	,tcon.TargetTableName
	,tcc.[Level] [Level]
...
DECLARE @FlatTableNameFlat NVARCHAR(1000) = (SELECT DISTINCT '[' + TargetSchema + '].[' + TargetTableName + ']' FROM #MetadataFlat)

SET @SQL = 'IF OBJECT_ID(''' + @FlatTableNameFlat + ''',''U'') IS NOT NULL DROP TABLE ' + @FlatTableNameFlat

------------------------------- RunSQL -------------------------------
EXEC [etl].[RunSQL]
	@BatchID = @BatchID
	,@TaskName = 'Drop And Create Destination Table'
	,@Target = @FlatTableNameFlat
	,@SQL = @SQL

DECLARE @WithClause NVARCHAR(MAX)

SELECT
	@WithClause = COALESCE(@WithClause + ',','') + '
	[' + REPLACE(REPLACE(ColumnName,'_',''),' ','') + '] ' + DataType +
	' ''$."' + Name_Raw + '"'''
FROM #MetadataFlat

SET @SQL = '
SELECT
	ROW_NUMBER() OVER(ORDER BY (SELECT 1)) [SK]
	,j1.ETL__multi_query_value_for_audit
	,j2.*
INTO ' + @TargetTableName + '
FROM
	(
	SELECT a.ETL__multi_query_value_for_audit,j.[Key],j.[Value]
	FROM
		(
		SELECT ETL__multi_query_value_for_audit,json_payload
		FROM ' + @StageTableName + '
		WHERE ETL__session_id = ''' + @ETL__session_id + '''
		) a
		CROSS APPLY OPENJSON(a.json_payload) j
	) j1
...
SET @SQL = '
SELECT TOP 100 -- to limit sessions
...
-- #New -- Only create a working table if there is more than 1 level of data
DECLARE @IsFlatHierarchy BIT = 0
--IF
--(
--SELECT
--	COUNT(DISTINCT [Level]) DistinctLevels
--FROM [etl].[JSON_Meta_Table_Configuration] tcon WITH (NOLOCK)
--	JOIN [etl].[JSON_Meta_TableColumn_Configuration] tccon WITH (NOLOCK) ON tcon.JSON_Meta_Table_Configuration_ID = tccon.JSON_Meta_Table_Configuration_ID
--	LEFT JOIN [etl].[JSON_Meta_TableColumn] tcc WITH (NOLOCK) ON tcon.JSON_Meta_Table_ID = tcc.JSON_Meta_Table_ID AND tccon.JSON_Meta_TableColumn_ID = tcc.JSON_Meta_TableColumn_ID
--WHERE ('[' + tcon.TargetSchema + '].[' + tcon.TargetTableName + ']' = @TargetTableName OR tcon.JSON_Meta_Table_Configuration_ID = @JSON_Meta_Table_Configuration_ID)
--	AND tccon.[Active] = 1
--) = 1
--SET @IsFlatHierarchy = 1

IF @IsFlatHierarchy = 0
BEGIN
...
SELECT @SQL += '
EXEC etl.JSONTranslator
	@Step = ''Create SRC Table' + CASE WHEN @IsFlatHierarchy =  1 THEN ' (Flat)' ELSE '' END + '''
...
/****** Object:  StoredProcedure [etl].[ReloadTasks]    Script Date: 7/18/2017 12:52:43 PM ******/
SET ANSI_NULLS ON
GO
